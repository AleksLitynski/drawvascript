	...
	var compiler = {
		...
		gen_target : {
			...
			invokable : function(labeled_graph, opts){
				...
				forEach(labeled_graph, function(node){
					event_graph[node.id] = event_graph[node.id] || event_node(node.value);
					if(opts.imports[node.value] instanceof Function){
						event_graph[node.id].on(opts.imports[node.value]);
					}
				})
	    ...
	    var self = {
	        ...
	        emit : function(new_data){
				for(var index in listeners){
					fireListener(listeners[index], new_data);
				}
		        data = new_data;
	            return self;
	        },
